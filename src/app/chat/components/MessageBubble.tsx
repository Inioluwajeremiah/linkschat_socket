...
function ReplyPreview({ replyTo, colors }: { replyTo: any; colors: any }) {
  ...
  return (
    ...
    >
      ...
      <Text
        style={[replyStyles.text, { color: colors.textPrimary }]}
        numberOfLines={1}
      >
        {preview}
      </Text>
          ...
          {!isDeleted && message.type === "sticker" ? (
            <View style={styles.stickerBubbleWrap}>
              ...
              <View style={styles.stickerMetaRow}>
                {message.isEdited && (
                  <Text
                    style={[styles.editedTag, { color: colors.textPrimary }]}
                  >
                    edited
                  </Text>
                )}
              ...
              {isForwarded && !isDeleted && (
                <View style={styles.forwardedLabel}>
                  <Ionicons
                    ...
                    size={11}
                    color={isOwn ? "rgba(255,255,255,0.6)" : colors.textPrimary}
                  />
                  <Text
                    style={[
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {message.callStatus === "missed"
                        ? `Missed ${
                            message.callType === "video" ? "video" : "voice"
                          } call`
                        : `${
                            message.callType === "video" ? "Video" : "Voice"
                          } call`}
                    ...
                    {message.callStatus !== "missed" && (
                      ...
                      >
                        {Math.floor((message.callDuration || 0) / 60)}:
                        {String((message.callDuration || 0) % 60).padStart(
                          2,
                          "0"
                        )}
                      </Text>
...
const callLogStyles = StyleSheet.create({
  ...
  duration: { fontSize: 12, marginTop: 1 },
  callBackBtn: {
    width: 30,
    height: 30,
    borderRadius: 15,
