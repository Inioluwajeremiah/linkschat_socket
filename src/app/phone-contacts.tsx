...
//           ]}
//           onPress={() =>
//             isRegisterScreen ? router.push("/(tabs)") : router.back()
//           }
//         >
//           {isRegisterScreen ? (
    ...
    <View style={styles.emptyWrap}>
      ...
      {!hasPermission ? (
        <>
          ...
          <TouchableOpacity
            style={styles.allowBtn}
            onPress={canAskAgain ? onRequest : onOpenSettings}
            activeOpacity={0.85}
          >
            <LinearGradient
              colors={[colors.primary, colors.primaryDark]}
            ...
            >
              ...
              />
              <Text style={[styles.allowText, { color: colors.textInverse }]}>
                {canAskAgain ? "Allow Contacts Access" : "Open Settings"}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
        </>
...
export default function PhoneContactsScreen() {
  ...
  // ── Single init effect ──────────────────────────────────────────────────
  // Previously this was two separate effects: one always called
  // checkPermissionAndLoad(), the other called requestPermission() when
  // isRegisterScreen was true. Both ran on mount in parallel — and since
  // requestPermissionsAsync() on an already-decided permission just
  // returns the current status without prompting, if permission was
  // already granted BOTH paths independently called loadAndSync(),
  // producing two concurrent syncs and two "Syncing contacts..." toasts.
  // Consolidated into one path so only one sync ever runs per mount.
  useEffect(() => {
    Animated.timing(headerAnim, {
      toValue: 1,
      duration: 500,
  ...
  const checkPermissionAndLoad = async () => {
    const { status, canAskAgain: askAgain } =
  ...
  const requestPermission = async () => {
    ...
    if (status === "granted") {
      ...
      // iOS won't re-show the system prompt once denied — only path
      // forward is the Settings app, so guide the user there instead of
  ...
  const handleInvite = async () => {
    try {
      await Share.share({
        message:
          "Join me on LinksChat! Download the app and let's start chatting: https://linkschat.app/download",
      });
    } catch {}
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>
            {isRegisterScreen ? "Find Your Friends" : "Friends on LinksChat"}
          </Text>
          {hasPermission && !loading && matchedContacts.length > 0 && (
            <Text style={[styles.headerSub, { color: colors.textMuted }]}>
              {matchedContacts.length} of {totalPhoneContacts} contacts
      ...
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            ...
            isRegisterScreen &&
              hasPermission &&
              matchedContacts.length > 0 && { paddingBottom: 100 },
          ]}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
          ...
          renderItem={({ item, index }) => (
            <ContactCard
              item={item}
              index={index}
              isCreatingChat={creatingChat === item._id}
              onChat={() => {
                if (creatingChat) return;
                handleChat(item._id);
              }}
      ...
      {/* Register flow: explicit path forward into the app */}
      {isRegisterScreen && !loading && (
        <View
          style={[
            styles.continueBar,
            {
        ...
        >
          ...
          >
            ...
            >
              <Text
                style={[styles.continueText, { color: colors.textInverse }]}
              >
                Continue to LinksChat
              </Text>
              <Ionicons
                name="arrow-forward"
                size={18}
