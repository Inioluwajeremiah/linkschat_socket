...
//       quality: 0.85,
//       videoMaxDuration: 60,
//       // Crop to a story-shaped frame at selection time, rather than
//       // letting the raw pick get force-cropped later at render time.
//       // Only applies to images — the system picker doesn't offer aspect
//       // cropping for video, so a selected video passes through as-is.
//       allowsEditing: true,
//       aspect: [9, 16],
//     });
//     if (result.canceled) return;
...
//     if (asset.type === "video") {
//       // ImagePickerAsset.duration is already in milliseconds — do not
//       // multiply by 1000 again below.
...
export default function StatusCreateScreen() {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Status.duration and Status.mediaDuration are both stored in
      // SECONDS on the backend (duration defaults to 5 — only sane as
      // seconds-to-display for text/image). mediaDurationMs is the video's
      // real length in milliseconds — convert once here rather than
      // sending a milliseconds value into a seconds-denominated field.
      const durationSeconds =
        mode === "video" ? Math.round(mediaDurationMs / 1000) : undefined;
