...
//     chat.type === "group"
//       ? { displayName: chat.name || "Unknown", isContact: false }
//       : resolveContact(otherParticipant?.phone, otherParticipant?.name);

//   // const displayName = chat.type === "group" ? chat.name : resolvedContactName;

//   // console.log(" displayName=>>> ", displayName);
//   // console.log("otherParticipant phone ===>>> ", otherParticipant?.phone);
//   // || otherParticipant?.name || "Unknown";

//   const routeLink =
//     chat.type === "group"
//       ? (`/group-info/${chat._id}` as RelativePathString)
//       : (`/profile/${otherParticipant?._id}` as RelativePathString);
...
//   const lstMsgCallType = lastMsg?.callType;
//   const lstMsgCallStatus = lastMsg?.callStatus;

//   let lstMsgCallText = "";

//   if (lstMsgCallType === "audio" && lstMsgCallStatus === "missed") {
//     lstMsgCallText = "Missed voice call";
//   } else if (lstMsgCallType === "video" && lstMsgCallStatus === "missed") {
//     lstMsgCallText = "Missed video call";
//   } else if (lstMsgCallType === "audio" && lstMsgCallStatus === "completed") {
//     lstMsgCallText = "Voice call";
//   } else if (lstMsgCallType === "video" && lstMsgCallStatus === "completed") {
//     lstMsgCallText = "Video call";
//   }
...
//       lastMsgText = "Sticker";
//       lastMsgIcon = "happy-outline";
//     } else if (lastMsg.type === "call") {
//       lastMsgText = lstMsgCallText || "Call";

//       if (lastMsg.callType === "audio") {
//         lastMsgIcon = "call-outline";
//       } else if (lastMsg.callType === "video") {
//         lastMsgIcon = "videocam-outline";
//       }
//     } else {
...
export default function ChatItem({
  ...
  // a name in the device address book — this doubles as the "saved
  // contact?" signal for the unsaved-contact badge below, so there's no
  // separate device-contacts lookup needed here.
  const { displayName, isContact: isContactSaved } =
    chat.type === "group"
      ? { displayName: chat.name || "Unknown", isContact: false }
      : resolveContact(otherParticipant?.phone, otherParticipant?.name);
