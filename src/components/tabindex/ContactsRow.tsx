...
// this cap — they always appear in full, ahead of everyone else.
const MAX_SUGGESTED_CONTACTS = 12;

type ContactsRowProps = {
  colors: any;
  contacts: User[];
  loading: boolean;
  chattedUserIds: Set<string>;
};
...
export default function ContactsRow({
  ...
  const visibleContacts = useMemo(() => {
    ...
    const chattedWith = withPhone.filter((c) => chattedUserIds.has(c._id));
    const notChattedWith = withPhone.filter((c) => !chattedUserIds.has(c._id));
    const remainingSlots = Math.max(
      MAX_SUGGESTED_CONTACTS - chattedWith.length,
      0
    );
    return [...chattedWith, ...notChattedWith.slice(0, remainingSlots)];
  }, [contacts, chattedUserIds]);
      ...
      {showMenu && menuContact && (
        ...
        >
          ...
          >
            ...
            >
              <Text
                style={[styles.menuCancelText, { color: colors.textPrimary }]}
              >
                Dismiss
              </Text>
            </TouchableOpacity>
